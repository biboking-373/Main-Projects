<script>
  ...
  async function loadCustomers() {
    try {
      loading = true;
      error = null;
      const response = await adminCustomersAPI.getAll({
        page: currentPage,
        limit: 20,
        search: searchQuery
      });
  ...
  async function saveCustomerDetails() {
    ...
    try {
      error = null;
      await adminCustomersAPI.update(selectedCustomer.user_id, editForm);
      closeDetailModal();
      await loadCustomers();
    } catch (err) {
  ...
  async function deleteCustomer(userId) {
    ...
    try {
      error = null;
      await adminCustomersAPI.delete(userId);
      await loadCustomers();
    } catch (err) {
...
<div class="container">
  <div class="header">
    ...
    <p>Manage customer details and information</p>
  </div>
            ...
            {#each customers as customer}
              <tr on:click={() => openDetailModal(customer)}>
                ...
                <td class="actions-cell">
                  ...
                  <button
                    on:click|stopPropagation={() => deleteCustomer(customer.user_id)}
                    class="btn-delete"
                  >
                    Delete
                  </button>
          ...
          {#if editMode}
            <div class="edit-form">
              ...
              <div class="form-actions">
                ...
                <button on:click={() => editMode = false} class="btn-cancel">
                  Cancel
                </button>
              </div>
            </div>
